...
2. **Per-Asset Analytics** — CAGR, simple/calendar-year returns, "Annualised Metrics by Look-back Period", **Max Drawdown** (always simple returns for consistency; optionally deflated to **real terms** — see the toggle above)
...
- `render_per_etf_analytics(..., real_terms=False, annual_inflation=0.0)` deflates each asset's price series when `real_terms`; `render_monte_carlo(..., real_terms=False)` / `render_scipy_ef(..., real_terms=False)` take `real_terms` only to print a nominal-basis caption (they never deflate — the frontier weights are inflation-invariant)
